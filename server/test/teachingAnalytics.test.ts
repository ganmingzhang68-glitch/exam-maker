...
test('teaching analytics persists snapshots and explains attention rules with role scoping', async () => {
  ...
  db.insert(schema.papers).values([{ id: 1, createdBy: 1, courseId: 1, title: '卷一', course: '统计学', totalScore: 100 }, { id: 2, createdBy: 1, courseId: 1, title: '卷二', course: '统计学', totalScore: 100 }, { id: 3, createdBy: 1, courseId: 1, title: '无效零分卷', course: '统计学', totalScore: 0 }]).run();
  db.insert(schema.exams).values([{ id: 1, paperId: 1, createdBy: 1, title: '考试一', status: 'published' }, { id: 2, paperId: 2, createdBy: 1, title: '考试二', status: 'published' }, { id: 3, paperId: 3, createdBy: 1, title: '无效考试', status: 'published' }]).run();
  db.insert(schema.examAssignments).values([{ id: 1, examId: 1, studentId: 2 }, { id: 2, examId: 2, studentId: 2 }, { id: 3, examId: 1, studentId: 3 }, { id: 4, examId: 2, studentId: 3 }, { id: 5, examId: 3, studentId: 2 }]).run();
  db.insert(schema.attempts).values([{ id: 1, examId: 1, assignmentId: 1, studentId: 2, status: 'graded', totalScore: 90, gradedAt: '2026-01-01T00:00:00Z' }, { id: 2, examId: 2, assignmentId: 2, studentId: 2, status: 'graded', totalScore: 50, gradedAt: '2026-02-01T00:00:00Z' }, { id: 3, examId: 3, assignmentId: 5, studentId: 2, status: 'graded', totalScore: 0, gradedAt: '2026-03-01T00:00:00Z' }]).run();
  ...
  try {
    const refreshed = await call('/courses/1/refresh', 1, 'POST'); assert.equal(refreshed.status, 201); assert.equal(refreshed.body.data.summary.participationRate, 0.6); assert.equal(refreshed.body.data.summary.averageScoreRate, 0.7, 'zero-total papers must not be averaged as zero performance');
    ...
    db.update(schema.teachingAnalyticsSnapshots).set({ summaryJson: '{broken' }).where(eq(schema.teachingAnalyticsSnapshots.id, refreshed.body.data.id)).run();
    const recovered = await call('/courses/1', 1); assert.equal(recovered.status, 200); assert.notEqual(recovered.body.data.id, refreshed.body.data.id, 'a corrupt snapshot must be replaced instead of crashing the dashboard');
