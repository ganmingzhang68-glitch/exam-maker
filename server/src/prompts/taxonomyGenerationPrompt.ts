...
export const taxonomyGenerationPrompt: PromptDefinition<typeof taxonomyGenerationInputSchema, typeof taxonomyGenerationOutputSchema> = {
  id: 'taxonomy_generation_prompt', version: '1.0.0', stage: 'knowledge_taxonomy_building',
  task: '只依据课程材料和历史题证据提出课程考点树。不得内置特定课程考点，不得覆盖 existingNodes 中 isLocked=true 的名称或父节点。',
  ...
  splitInput: input => ({ trustedContext: { course: input.course, existingNodes: input.existingNodes }, untrustedData: { materialSummaries: input.materialSummaries, questions: input.questions } }),
  examples: {
